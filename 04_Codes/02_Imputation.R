...
date.continuity <- total.raw %>% 
  distinct(province, city, pchc, year, date) %>% 
  count(province, city, pchc, year) %>% 
  setDT() %>% 
  dcast(province + city + pchc ~ year, value.var = "n", fill = 0) %>% 
...
city.growth <- date.continuity %>% 
  filter(cnt_min >= 2) %>% 
  inner_join(total.raw, by = c("province", "city", "pchc")) %>% 
  group_by(province, city, year, atc4, molecule_desc) %>% 
  summarise(sales = sum(sales, na.rm = TRUE),
            units = sum(units, na.rm = TRUE)) %>% 
  ungroup() %>% 
  setDT() %>% 
  melt(id.vars = c("province", "city", "year", "atc4", "molecule_desc"), 
       measure.vars = c("units", "sales"), variable.name = "cate", value.name = "value") %>% 
  unite("type", cate, year) %>% 
  setDT() %>% 
  dcast(province + city + atc4 + molecule_desc ~ type, value.var = "value", fill = 0) %>% 
  ...
  select(province, city, atc4, molecule_desc, sales_growth, units_growth)

# imputing
imputing.data <- date.continuity %>% 
  filter(cnt_max >= 2) %>% 
  select(province, city, pchc) %>% 
  left_join(total.raw, by = c("province", "city", "pchc")) %>% 
  mutate(month = stri_sub(date, 5, 6)) %>% 
  setDT() %>% 
  melt(id.vars = c("province", "city", "pchc", "year", "month", "atc4", "molecule_desc", "packid"),
       measure.vars = c("units", "sales"), variable.name = "cate", value.name = "value") %>% 
  unite("type", cate, year) %>% 
  dcast(province + city + pchc + atc4 + molecule_desc + packid + month ~ type,
        value.var = "value", fill = -1) %>%
  left_join(city.growth, by = c("province", "city", "atc4", "molecule_desc")) %>% 
  ...
  setDT() %>% 
  melt(id.vars = c("province", "city", "pchc", "atc4", "molecule_desc", "packid", "month"), 
  ...
  dcast(province + city + pchc + year + month + atc4 + molecule_desc + packid ~ type, 
        value.var = "value") %>% 
  rename("units_imp" = "units",
         "sales_imp" = "sales") %>% 
  mutate(date = stri_paste(year, month),
         quarter = stri_paste(year, "Q1")) %>% 
  select(year, date, quarter, province, city, pchc, atc4, molecule_desc, 
